...
router.post('/update-student/:id', function(req, res, next) {
  ...
  console.log('The student to be removed is: ', productId);
  Product.findById(studentId)
  .exec(function(err, product) {
      if(err) {
        ...
        product.studentLetterGrade = letterGrade;

        product.save(function(err, updatedStudent) {
          if(err) {
